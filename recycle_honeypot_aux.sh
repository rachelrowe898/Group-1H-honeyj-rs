...
if [ "$mitm_port" == "10000" ]; then
  sudo bash recycle_honeypot.sh "$honeypot" 128.8.238.119 26 10000 /home/student/active_data_0 "$mitm_port" >> /home/student/honeypot_logs/0_lifecycle.log 2>&1
elif [ "$mitm_port" == "10001" ]; then 
  sudo bash recycle_honeypot.sh "$honeypot" 128.8.238.86 26 10001 /home/student/active_data_1 "$mitm_port" >> /home/student/honeypot_logs/1_lifecycle.log 2>&1
elif [ "$mitm_port" == "10002" ]; then 
  sudo bash recycle_honeypot.sh "$honeypot" 128.8.238.120 26 10002 /home/student/active_data_2 "$mitm_port" >> /home/student/honeypot_logs/2_lifecycle.log 2>&1
elif [ "$mitm_port" == "10003" ]; then 
  sudo bash recycle_honeypot.sh "$honeypot" 128.8.37.125 27 10003 /home/student/active_data_3 "$mitm_port" >> /home/student/honeypot_logs/3_lifecycle.log 2>&1
else
